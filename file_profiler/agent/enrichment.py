...
async def enrich(
    ...
    1. Build documents from profiles + relationships + sample rows
    2. Embed into ChromaDB
    3. Retrieve all context (small corpus, full retrieval)
    ...
    Returns:
        Dict with ``enrichment`` (full LLM analysis text) and
        ``documents_embedded`` count.
    """
    from file_profiler.agent.llm_factory import get_llm_with_fallback

    # Step 1: Build documents
    documents = build_documents(profiles, report, dir_path)

    # Step 2: Embed into vector store (HuggingFace all-MiniLM-L6-v2)
    vector_store = create_vector_store(documents)

    # Step 3: Retrieve context
    # For small corpora (< 50 tables), retrieve everything.
    # For larger corpora, use similarity search.
    if len(documents) <= 50:
        context_docs = documents
    else:
        retriever = vector_store.as_retriever(search_kwargs={"k": 30})
        context_docs = await retriever.ainvoke(
            "Describe all tables, their columns, relationships, "
            "primary keys, foreign keys, and data quality issues."
        )

    context_text = "\n\n---\n\n".join(doc.page_content for doc in context_docs)

    # Step 4: LLM enrichment (falls back to Groq if Google quota exhausted)
    llm = get_llm_with_fallback(provider=provider, model=model)
    prompt = ENRICHMENT_PROMPT.format(context=context_text)

    log.info("Sending enrichment prompt to LLM (%d chars context)", len(context_text))
    ...
    # Cleanup: delete the transient collection
    try:
        vector_store.delete_collection()
        log.debug("Transient vector store collection deleted")
    except Exception:
        pass

    log.info("Enrichment complete (%d chars)", len(content))
